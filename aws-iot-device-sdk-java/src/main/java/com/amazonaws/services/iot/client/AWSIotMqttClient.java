...
public class AWSIotMqttClient extends AbstractAwsIotClient {
     ...
     *            {@code keyStore} argument.
     * @param root
     *            the key store containing the client X.509 root certificates to
     *            be used to authenticate the sever. The {@link KeyStore} object
     *            can be constructed using the root-ca.pem X.509 certificate file
     *            for talking to the cloud or the group ca returned by the discovery
     *            API if talking to a Green Grass core
     */
    public AWSIotMqttClient(String clientEndpoint, String clientId, KeyStore keyStore, String keyPassword, KeyStore root) {
        super(clientEndpoint, clientId, keyStore, keyPassword, root);
    }
